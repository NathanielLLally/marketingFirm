...
async def scrape_profile(urls: List[str]) -> List[Dict]:
    ...
    async for response in SCRAPFLY.concurrent_scrape(to_scrape):
        with open(output.joinpath("result.json"), "w", encoding="utf-8") as file:
            dumper.dump(response,file)
        try:
...
def parse_company_directory(response: ScrapeApiResponse) -> Dict:
...
async def scrape_company_directory(urls: List[str]) -> List[Dict]:
    ...
    async for response in SCRAPFLY.concurrent_scrape(to_scrape):
        try:
            ...
            company_name = str(response.context["url"]).split("/")[-1]
            directory_page_url = f"https://www.linkedin.com/company/{company_name}?trk=companies_directory"
            # request the company life page
            page_response = await SCRAPFLY.async_scrape(ScrapeConfig(directory_page_url, **BASE_CONFIG))
            companies = parse_company_directory_page(page_response)
            data.append({"companies": companies})
        except Exception as e:
            log.error("An occured while scraping company directory pages", e)
            pass
