...
our $DEBUG = 1;
LWP::Debug::level('+trace', '+debug'); # Enable trace and debug messages
...
if ($opt->{mode} eq 'poll') {
  ...
  print prettyDump($res->{dids}, $keys);
  exit;
...
sub restCall {
    ...
    my $q = join ('&', "api_username=$api_username", "api_password=".$URI->encode($api_password),
	 map { sprintf("$_=%s", $URI->encode($data{$_})); } sort keys %data);
    print "query=[$q]\n" if ($DEBUG);

    my $req = HTTP::Request->new('GET', "https://voip.ms/api/v1/rest.php?$q");
