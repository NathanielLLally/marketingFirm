...
my $table = 'track_email_test';

my $sth = $dbh->prepare ("select count($table.uuid) from $table where pending is not null and sent is null");
$sth->execute;
my $res = $sth->fetchall_arrayref();
my $pending = $res->[0]->[0];
my $limit = 30 - $pending;

$sth = $dbh->prepare ("update $table set pending = now() where $table.uuid in (select $table.uuid from $table where sent is null and pending is null limit $limit)");
$sth->execute;
$sth = $dbh->prepare ("select email,uuid,name from $table where pending is not null and sent is null");
$sth->execute;
my $batch = $sth->fetchall_arrayref({});


my $threads = ($#{$batch} > 3) ? 3 : $#{$batch};
$threads = 0;
if ($threads < 0) {
  ...
  foreach (@random) {
    ...
    if (defined $status) {
      if ($status isa "Email::Sender::Success") {
        if (Role::Tiny::does_role($status, "Email::Sender::Role::HasMessage")) {
          print $status->{message};
        } else {
          print Dumper(\$status);
        }
        my $isth = $dbh->prepare ("update $table set sent = now(),subject = ?, content_url = ? where uuid = ?");
        ...
        try {
          $isth = $dbh->prepare ("insert into mx_email (name,did) select ? as name, id as did from mx_domain where domain = ?");
