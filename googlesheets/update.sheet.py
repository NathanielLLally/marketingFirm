...
def main():
  ...
  sql = "select distinct yp.name,yp.phone,string_agg(yp.category,',') as categories,yp.website,e.email,yp.address,yp.city,yp.state,yp.zip from yp.yellow_pages yp join email e on yp.website = e.website group by name,phone,e.email,yp.website,yp.address,yp.city,yp.state,yp.zip"
  rs = sheet.db_fetch(f"select count(*) from ({sql})")
  ranges = 'name_email_sample!C1:D'
  ...
  rs = sheet.db_fetch(f"select * from ({sql}) where random() < 0.001 limit 1000")
#rs = sheet.db_fetch("select * from yp.yellow_pages limit 10;")
