...
sub Net::Whois::Raw::whois_query {
    ...
    sub blacklist_srv
    {
        ...
        local $/ = undef;
        print "blacklist_srv($srv,$time)";
        my $then = $now + 24 * 3600 + 1;
        if (defined $time) {
            if ($time =~ /((\d+)h)?((\d+)m)?((\d+)s)?/) {
                my ($h, $m, $s) = ($2 || 0,$4 || 0,$6 || 0);
                $then = $h * 3600 + $m * 60 + $s + $now + 1;
            } elsif ($time =~ /^\d+$/) {
                $then = $now + $time + 1;
            }
        }
        my $tstr = localtime($then);
        print "=> $tstr\n";
        $srv{$srv} = $then;
    }
    ...
    sub is_blacklisted
    {
        ...
        if (exists $srv{$srv}) {
            ...
            my $then = localtime($srv{$srv});
            print "$srv blacklisted until $then\n";
            return 1;
        }
        return 0;
    }
